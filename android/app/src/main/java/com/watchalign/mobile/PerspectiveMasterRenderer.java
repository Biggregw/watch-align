...
/**
 * Deterministic model-master renderer used by the manual perspective workbench.
 * The QC photo never changes marker geometry. User controls only camera pose,
 * centre, scale and overlay opacity.
 */
final class PerspectiveMasterRenderer {
    static final class Pose {
        ...
        Pose copy(){
            Pose p=new Pose();p.centerX=centerX;p.centerY=centerY;p.scalePx=scalePx;
            p.pitchDeg=pitchDeg;p.yawDeg=yawDeg;p.rollDeg=rollDeg;p.alpha=alpha;return p;
        }
    }
    ...
    private static PointF project(Pose p,double x,double y){
        ...
        // Plane starts at z=0. Rotate in 3-D, then perspective-project with a fixed camera distance.
    ...
    private static void drawCircle(Canvas c,Pose p,double r,Paint paint){
        Path path=new Path();for(int i=0;i<=180;i++){double a=2*Math.PI*i/180.0;PointF q=project(p,r*Math.cos(a),r*Math.sin(a));if(i==0)path.moveTo(q.x,q.y);else path.lineTo(q.x,q.y);}c.drawPath(path,paint);
    }
    private static void drawRound(Canvas c,Pose p,double rr,double size,double a,Paint paint){
        double cx=rr*Math.cos(a),cy=rr*Math.sin(a);Path path=new Path();for(int i=0;i<=48;i++){double q=2*Math.PI*i/48.0;PointF z=project(p,cx+size*Math.cos(q),cy+size*Math.sin(q));if(i==0)path.moveTo(z.x,z.y);else path.lineTo(z.x,z.y);}path.close();c.drawPath(path,paint);
    }
    private static void drawRect(Canvas c,Pose p,double rr,double tang,double radial,double a,Paint paint){
        double cx=rr*Math.cos(a),cy=rr*Math.sin(a),ux=Math.cos(a),uy=Math.sin(a),vx=-uy,vy=ux;
        double[][] pts={{cx-ux*radial-vx*tang,cy-uy*radial-vy*tang},{cx-ux*radial+vx*tang,cy-uy*radial+vy*tang},{cx+ux*radial+vx*tang,cy+uy*radial+vy*tang},{cx+ux*radial-vx*tang,cy+uy*radial-vy*tang}};drawPoly(c,p,pts,paint);
    }
    private static void drawTriangle(Canvas c,Pose p,Paint paint,boolean lume){
        ...
        double half=lume?Gmt126710BlnrMaster.TRI_LUME_HALF_BASE:Gmt126710BlnrMaster.TRI_HALF_BASE;
        double cx=center*ux,cy=center*uy;
        double[][] pts={{cx+ux*baseOut+vx*half,cy+uy*baseOut+vy*half},{cx+ux*baseOut-vx*half,cy+uy*baseOut-vy*half},{cx-ux*apexIn,cy-uy*apexIn}};drawPoly(c,p,pts,paint);
    }
