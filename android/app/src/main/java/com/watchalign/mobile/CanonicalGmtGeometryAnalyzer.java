...
final class CanonicalGmtGeometryAnalyzer {
    ...
    private static Measure measure(Bitmap bitmap){
        ...
        try{
            ...
            double q=dial.quality,dr=dial.r;if(!(dr>0))return null;
            ...
            for(DialAnalysisEngine.Marker x:markers){
                int h=x.hour;if(h<1||h>12)continue;
                double radius=x.radius;
                m.present[h]=true;
                m.angularResidualDeg[h]=x.angular;
