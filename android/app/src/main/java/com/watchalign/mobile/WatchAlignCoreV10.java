...
/**
 * Alpha10 core. QC is run once without a reference so the retired V7 raster
 * registration/validation path cannot affect the result. Reference comparison
 * is handled only by GeometryOverlayRepair's deterministic geometry transform.
 */
public final class WatchAlignCoreV10 {
    public static final String CORE_VERSION = "1.3.0-alpha10";
    ...
    public static AnalysisResult analyse(Bitmap watch, Bitmap reference, String modelRef) {
        // Deliberately pass null: V7 remains the proven QC detector/measurement
        // engine, but its old raster post-warp validation is not used anymore.
