...
final class CaptureGuideView extends View {
    ...
    @Override protected void onDraw(Canvas c){super.onDraw(c);float r=Math.min(getWidth(),getHeight())*.30f,cx=getWidth()/2f,cy=getHeight()/2f;paint.setColor(ready?Color.rgb(60,255,120):Color.WHITE);paint.setAlpha(210);c.drawCircle(cx,cy,r,paint);c.drawLine(cx-24,cy,cx+24,cy,paint);c.drawLine(cx,cy-24,cx,cy+24,paint);c.drawLine(cx,cy-r-15,cx,cy-r+22,paint);}
}
