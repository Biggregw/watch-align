...
final class QcMeasurementFormatter {
    ...
    static String triangleTrackPosition(float baseTo60){
        float ref=GenTriangle12RelationalReference.BASE_TO_60_INNER_OVER_BASE;
        float delta=baseTo60-ref;
        if(Math.abs(delta)<0.0005f)return String.format(Locale.US,"12 triangle position: at projected reference gap %.3f BW",ref);
        String relation=delta<0?"closer to minute track":"farther from minute track";
        return String.format(Locale.US,"12 triangle position: %s than projected reference by %.3f BW",relation,Math.abs(delta));
    }
