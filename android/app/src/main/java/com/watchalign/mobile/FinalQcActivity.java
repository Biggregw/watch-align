...
/** Final GMT QC: one perspective-projected ideal geometry model plus measured residuals. */
public class FinalQcActivity extends Activity {
    ...
    private void buildAndShowUi(final GmtQcPipeline.Result result,final GmtIndexAutoAnalyzer.Result indexResult,String failure){
        ...
        TextView summary=txt(qcSummary,9);summary.setGravity(Gravity.CENTER);bottom.addView(summary,new LinearLayout.LayoutParams(-1,dp(190)));
        ...
        Button unified=btn("Ideal + measured");unified.setOnClickListener(v->image.setImageBitmapPreserveZoom(combined));row.addView(unified,new LinearLayout.LayoutParams(0,dp(46),1));
        ...
        TextView hint=txt("YELLOW = where the 126710 geometry should be after perspective projection. MAGENTA = measured watch geometry. The automatic 12 triangle is part of this same model; manual 12 placement is no longer required for normal GMT QC.",9);hint.setGravity(Gravity.CENTER);bottom.addView(hint,new LinearLayout.LayoutParams(-1,dp(58)));
        root.addView(bottom,new LinearLayout.LayoutParams(-1,dp(348)));
    }
    ...
    private String buildSummary(GmtQcPipeline.Result result,GmtIndexAutoAnalyzer.Result indexResult,String failure){
        ...
        String indices=indexResult==null?"IDEAL GMT GEOMETRY\nUnavailable.":indexResult.summary;
        return triangle+"\n\n"+indices;
    }
