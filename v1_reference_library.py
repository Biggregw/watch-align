...
def list_references(backend, model_ref: str) -> list[dict[str,Any]]:
    root=_root(backend,model_ref); items=[]
    for path in sorted(root.iterdir()):
        ...
        if mp.exists():
            try: meta.update(json.loads(mp.read_text(encoding="utf-8")))
...
def choose_reference(backend, model_ref: str):
    """Prefer trusted persistent references, then packaged references."""
    root=_root(backend,model_ref); items=list_references(backend,model_ref)
    ordered=sorted(items,key=lambda x:(not bool(x.get("trusted")),x.get("filename","")))
    for meta in ordered:
        path=root/meta["filename"]; image=cv2.imread(str(path))
        if image is not None:
            status="user-trusted stored reference" if meta.get("trusted") else "unverified stored reference"
            return backend.resize_max(image),f"{path.name} ({status})"
...
def install_reference_library(backend, v1_full_module) -> None:
    if getattr(backend,"_watch_align_reference_library_installed",False):return
    backend._watch_align_reference_library_installed=True
    # v1_full resolves this global at request time, so Gen Compare immediately
    # gains access to the persistent library without duplicating the analysis API.
    v1_full_module._built_in_reference=choose_reference

    @backend.app.get("/v1/references",response_class=HTMLResponse)
    def reference_manager(): return HTMLResponse(REFERENCE_MANAGER_HTML)

    @backend.app.get("/api/v1/references/{model_ref}")
    def references(model_ref: str):
        if model_ref not in v1_full_module.MODEL_GEOMETRY: raise HTTPException(status_code=404,detail="Unknown model")
    ...
    def add_reference(model_ref: str,image: UploadFile=File(...),source: str=Form(""),trusted: bool=Form(False)):
        ...
        if len(raw)>backend.MAX_UPLOAD_BYTES: raise HTTPException(status_code=413,detail="Reference image is too large")
        decoded=cv2.imdecode(__import__('numpy').frombuffer(raw,dtype=__import__('numpy').uint8),cv2.IMREAD_COLOR)
        if decoded is None: raise HTTPException(status_code=422,detail="Reference file is not a readable image")
        root=_root(backend,model_ref);digest=hashlib.sha256(raw).hexdigest();name=f"{int(time.time())}-{digest[:10]}-{_safe_name(image.filename or 'reference.jpg')}";path=root/name
        path.write_bytes(raw);meta={"filename":name,"source":source.strip()[:500],"trusted":bool(trusted),"verification":"user-asserted" if trusted else "unverified","added_at":time.strftime('%Y-%m-%d %H:%M:%S'),"sha256":digest,"original_filename":image.filename}
