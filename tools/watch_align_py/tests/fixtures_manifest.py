...
CASES = [
    ...
    FixtureCase(
        ...
        known_issue=(
            ...
            "\n\n"
            "2026-09-23 addendum: this same photo, decoded via the research-tooling path "
            "(PIL EXIF-transpose + LANCZOS resize to 1800px, used by "
            "tools/research/run_replica_control_set.py -- NOT this test's decode_capped, "
            "which mirrors production and still gets expect_pose_accepted=True above) "
            "triggers the identity gate: automatic_accepted=True but centre displacement "
            "vs the legacy seed detector is 80% of dial radius (suspicion threshold 22%), "
            "and identity_gate.marker_good() finds 0/3 of 12/6/9 because each marker's p90 "
            "brightness (141-155) falls just under the gate's fixed 165 threshold -- "
            "contrast (109-131) and bright_fraction both pass comfortably, consistent with "
            "the same dim-exposure issue already documented above for marker_qc's older "
            "fixed threshold. See docs/research/gmt-genuine-baseline-integration-validation-"
            "2026-09-23-identity-bypass.md for the full diagnosis and the research-only "
            "provenance-verified bypass this project uses instead of changing "
            "identity_gate.py's threshold, and "
            "docs/research/gmt-replica-control-set-results/pose_gate_diagnostics.csv for "
            "the raw numbers."
        ),
    ),
]
